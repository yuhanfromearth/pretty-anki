    ...
    <div className="relative overflow-hidden rounded-2xl bg-milk-100/80 border border-milk-200/60 px-6 py-5 shadow-soft">
      ...
      <div className="relative flex items-center justify-between gap-4">
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div>
            <h2 className="text-2xl font-semibold text-ink-900">{deck.name}</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {deck.learnCount > 0 && (
              <span className="rounded-full border border-apricot/40 px-3 py-0.5 text-xs font-medium text-apricot">
                {deck.learnCount} learning
              </span>
            )}
            {deck.reviewCount > 0 && (
              <span className="rounded-full border border-mint-300 px-3 py-0.5 text-xs font-medium text-mint-700">
                {deck.reviewCount} review
              </span>
            )}
            {deck.newCount > 0 && (
              <span className="rounded-full border border-lilac/40 px-3 py-0.5 text-xs font-medium text-lilac">
                {deck.newCount} new
              </span>
            )}
          </div>
          <button className="inline-flex items-center gap-2 rounded-full bg-mint-500 px-5 py-2.5 text-sm font-medium text-white shadow-soft transition-colors hover:bg-mint-700">
            Start review
