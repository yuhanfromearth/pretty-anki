      ...
      <div className="relative flex items-center justify-between gap-4">
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {deck.learnCount > 0 && (
              <span className="rounded-full border border-apricot/40 bg-apricot/10 px-3 py-0.5 text-xs font-medium text-[#b8884e]">
                {deck.learnCount} learning
              </span>
            )}
            {deck.reviewCount > 0 && (
              <span className="rounded-full border border-mint-300 bg-mint-500/10 px-3 py-0.5 text-xs font-medium text-mint-700">
                {deck.reviewCount} review
              </span>
            )}
            {deck.newCount > 0 && (
              <span className="rounded-full border border-lilac/40 bg-lilac/10 px-3 py-0.5 text-xs font-medium text-[#9177c0]">
                {deck.newCount} new
              </span>
            )}
          </div>
          <button className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-mint-500 px-5 py-2.5 text-sm font-medium text-white shadow-soft transition-colors hover:bg-mint-700">
            Start review
